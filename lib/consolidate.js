...
/**
 * Fail silently if
 * ['view options'].failSilentlyIfMatches
 * is set.
 */

function failSilently(options, path) {
  var failSilentlyOptions = options.settings['view options'].failSilentlyIfMatches;

  if (failSilentlyOptions && Array.isArray(failSilentlyOptions)) {
    if (failSilentlyOptions.some(function (item) { return path.indexOf(item) === 0 })) {
      return true;
    }
  }
  return false;
